...
def load_config_from_package(filename: str) -> dict[str, Any]:
    with resources.files("toy_catalogue.config.settings").joinpath(filename).open(
        "rb"
    ) as f:
        return tomli.load(f)
...
def build_settings(
    ...
    registry_map = {
        ...
        "middlewares_downloader": "DOWNLOADER_MIDDLEWARES",
        "middlewares_spider": "SPIDER_MIDDLEWARES",
